...
def build_model(hp):
    model = Sequential()
    model.add(LSTM(hp.Int("input_units", 32, 256, 32), input_shape=(X.shape[1], X.shape[2]), return_sequences=True))
    model.add(Dropout(0.2))
    for i in range(hp.Int("n_layers", 1, 4)):
        model.add(LSTM(hp.Int(f"conv_{i}_units", 32, 256, 32), return_sequences=True))
        model.add(Dropout(0.2))
    model.add(LSTM(128))
    model.add(Dropout(0.2))
